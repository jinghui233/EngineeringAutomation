...
def calculate_inner_length():
    ...
    # plt.imshow(cross_region_points_img)
    # plt.show()
    seg_line_img = cv2.absdiff(sketech_img, cross_region_points_img)
    # plt.imshow(seg_line_img)
    # plt.show()

    contours, hierarchy = cv2.findContours(seg_line_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    print('all contours num:', len(contours))
    draw_line_img = np.zeros(shape=seg_line_img.shape)
    for i in range(len(contours)):
        cv2.drawContours(draw_line_img, contours, i, color=(i), thickness=1)
    draw_line_img_dilate = cv2.dilate(draw_line_img, kernel=cv2.getStructuringElement(shape=cv2.MORPH_RECT, ksize=(5, 5)))
    # cv2.imwrite('seg_line_img.jpg',seg_line_img*255)
    cv2.imwrite('draw_line_img.jpg', draw_line_img * 255)
    inner_line_img = draw_line_img * draw_line_img_dilate
    plt.imshow(inner_line_img)
    plt.show()
    # get line end points
...
def generate_luodai():
    ...
    # for i in range(len(contours)):
    #     cv2.drawContours(inner_draw_img, contours, i, color=(255), thickness=1)
    # plt.imshow(inner_draw_img)
    plt.show()


if __name__ == '__main__':
    _, _ = calculate_inner_length()
