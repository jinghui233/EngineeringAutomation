...
class AnalyticGeometry:
    ...
    @staticmethod
    def PointIsInLine(line: gbLine, point: tuple):
        if np.linalg.norm(np.array([line.end[0] - point[0], line.end[1] - point[1]])) < line.aperture.radius:
            return True, line.end
        if np.linalg.norm(np.array([line.start[0] - point[0], line.start[1] - point[1]])) < line.aperture.radius:
        ...
        if crossDistance > 0 and crossDistance < lineLength and distance < line.aperture.radius:
            return True, crossPoint
        return False, None

    @staticmethod
    def PointToLine(line: gbLine, point: tuple):  # 点到线的距离，交点坐标，起点到交点的距离，线长度 参考https://blog.csdn.net/tracing/article/details/46563383
    ...
    @staticmethod
    def get_line_cross_point(line1: gbLine, line2: gbLine):
        # x1y1x2y2
...
class AnalyticLine:
    def __init__(self, line: gbLine):
        ...
        self.ex = line.end[0]
        self.ey = line.end[1]
        self.thickness = line.aperture.radius
