...
class LineSet:
    ...
    def addLine(self, line: LinePice):
        if (self.linesNum == 0 or self.__lines[self.linesNum - 1].end == line.start) and self.linesNum == 0:
            self.__addLine(line)
            return True
        else:
            return False
    ...
    def Regenerate(self):
        ...
        for line in self.__lines:
            ...
            if line.setCut == SetCut.Start and line.signType!=SignType.Cut:
                newLines = LineSet()
                newLines.__addLine(line)
                newLinesSets.append(newLines)


            if line.setCut == SetCut.End and line.signType!=SignType.Cut:
                newLines.__addLine(line)
                newLines = LineSet()
                newLinesSets.append(newLines)
